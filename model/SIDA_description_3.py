    ...
    def model_forward(
        ...
        **kwargs,
        ...
        cls_token_mask = torch.cat([
            cls_token_mask,
            torch.zeros((cls_token_mask.shape[0], 1), dtype=torch.bool, device=input_ids.device)    # 將 tensor 創立在要運算的 GPU 上
            ], 
            dim=1)
        cls_token_mask =  torch.cat([
            torch.zeros((cls_token_mask.shape[0], 255), dtype=torch.bool, device=input_ids.device),  
            cls_token_mask,
            ], dim=1,)
        ...
        if (cls_labels == 2).any():
            ...
            seg_token_offset = torch.cat(
                [torch.zeros(1, dtype=torch.long, device=seg_token_counts.device), seg_token_offset], dim=0
            )
            try:
