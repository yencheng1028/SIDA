...
def main(args):
    ...
    if args.load_in_4bit:
        kwargs.update(
            {
                # 這裡改成 float32，讓輸入數據配合計算精度
                "torch_dtype": torch.float32,
                "load_in_4bit": True,
                "quantization_config": BitsAndBytesConfig(
                    load_in_4bit=True,
                    # 關鍵修改：Pascal 顯卡必須用 float32 進行計算
                    bnb_4bit_compute_dtype=torch.float32,  # 改成 float32
                    # 建議改成 False，Double Quant 在舊顯卡上有時會導致形狀錯誤
                    bnb_4bit_use_double_quant=False,
                    bnb_4bit_quant_type="nf4",
                    # 這裡加入 "mm_projector" 確保它不被量化導致維度崩壞
    ...
    # 如果使用了 4-bit 或 8-bit 量化，跳過手動 dtype 轉換
    if not (args.load_in_4bit or args.load_in_8bit):
        ...
        # 量化模型只需要確保它在 GPU 上（雖然 from_pretrained 通常處理好了，但保險起見）
        model = model.cuda()
    ...
    while True:
        ...
        # 無論 precision 參數為何，如果用了量化載入，輸入必須是 float32 以配合計算精度
