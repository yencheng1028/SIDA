...
4. Inference指令: CUDA_VISIBLE_DEVICES=0,1 python chat_descriptionDualGpu.py --version='./ck/SIDA-7B-description' --precision fp16

5. Hugging Face Accelerate 的 device_map="auto" 功能: 自動將模型的不同層「切塊」，平均分配到兩張 1080 Ti, conda install -c conda-forge accelerate

6. 安裝步驟: 
    1. 建立環境
    conda create -n SIDA python=3.9 -y
    conda activate SIDA
